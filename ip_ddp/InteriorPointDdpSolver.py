...
from ip_ddp.BackwardPass import BackwardPass
from ip_ddp.ForwardPass import ForwardPass
...
class InteriorPointDdpSolver:
    def __init__(
        ...
        self.traj: TrajInfo = traj
        self.traj_pre: TrajInfo = copy.deepcopy(traj)
        ...
        # Running iLQR for the initial few steps can often improve computation time and optimization stability, especially when the initial state is not fixed.
        self.start_ddp_cnt: int = 5
    ...
    def _eval_terminate_on_consition(
    ...
    def _iterate(self) -> None:
        for iter in range(self.max_iter):
            ...
            bp_result = self.bp.update(
                self.traj, self.fp.failed, self.barrier_param, use_hessian=use_hessian
            )
            ...
            if self._eval_terminate_on_consition(
